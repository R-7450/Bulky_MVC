...
namespace BulkyWeb.Controllers
{
    public class CategoryController : Controller
    {
        ...
        {
           List<Category>  objCategoryList = _categoryRepo.GetAll().ToList(); // command is used all the records from category table
            ...
            if (ModelState.IsValid)
             {
                ...
                _categoryRepo.Save(); // save all the changes made in this context to database
        ...
        public IActionResult Edit(int? id)  // if something is not mention by default this will be Get  Action
        {
            ...
            Category? categoryFromDb = _categoryRepo.Get(u=>u.Id==id); // find method work on only primary key only
        ...
        {
            Category? obj= _categoryRepo.Get(u => u.Id == id);// since both the method have same name thats why we name this method as deletepost
            if(obj==null)
            {
                return NotFound();
            }
